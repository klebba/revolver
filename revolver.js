...
var Revolver = function(element, options) {
    ...
    var m = typeof Modernizr == 'undefined' ? false : Modernizr;
...
Revolver.prototype = {
    ...
    syncPosition: function(time) {
        ...
        var state = this.state,
            ...
            time = typeof time == 'undefined' ? this.options.speed : time;
    ...
    focusItem: function(target) {
        ...
        if(!lastTarget || target.get(0) !== lastTarget.get(0)) {
            ...
            target.prev().addClass(beforeFocusClass);
            target.next().addClass(afterFocusClass);
    ...
    detectHitAreaHit: function() {
        ...
        var hit     = this.getHitArea(),
            ...
            minX    = parseInt(hit[0]),
            minY    = parseInt(hit[1]),
            maxX    = minX + parseInt(hit[2]),
            maxY    = minY + parseInt(hit[3]);
    ...
    onTouchStart: function(event) {
        ...
        var event  = this.normalizeTouchEvent(event),
            startX = event.pageX,
            startY = event.pageY;
    ...
    onTouchMove: function(event) {
        ...
        var event    = this.normalizeTouchEvent(event),
            ...
            touch    = this.touch,
            state    = this.state,
            deltaX   = touch.startX - event.pageX,
            deltaY   = touch.startY - event.pageY,
            lastX    = event.pageX,
            lastY    = event.pageY,
    ...
    calculateFirstVisibleItemIndexAtPosition: function(position) {
        ...
        return self.calculateItemIndexAtPosition(edge);
    },
    ...
    calculateLastVisibleItemIndexAtPosition: function(position) {
        ...
        var state    = this.state,
            overflow = state.maskSize % state.itemSize,
    ...
    calculatePositionAtItemIndex: function(index) {
        ...
        return Math.floor((index * state.itemSize) * -1);
    },
    ...
    calculatePages: function(items, itemsPerPage) {
        ...
        var items     = items.not('.' + this.options.classNames.itemClone); // ignore clones
            numItems  = items.length,
    ...
    move: function(position, time, easing) {
        ...
        var self            = this,
            ...
            easing          = typeof easing == 'undefined' ? options.easing : easing,
        ...
        if(position === currentPosition) {
            ...
            if(options.infinite && !this.infiniteFlag) {
                ...
                if(jump !== position) {
                    ...
                    this.infinite.nextTime = time;
                    this.infinite.nextEasing = easing;
        ...
        if(options.transitions) {
            ...
            this.applyVendorStyle(container, 'transition-duration', time + 'ms');
            this.applyVendorStyle(container, 'transition-timing-function', easing);
            ...
            if(options.transforms || options.transform3d) {
                ...
                this.applyVendorStyle(container, 'transition-property', time > 0 ? 'transform' : 'none', true);
            } else {
                this.applyVendorStyle(container, 'transition-property', time > 0 ? 'left' : 'none');
            }
            ...
            container.stop().animate({ left: position }, time, easing, $.proxy(this.afterMove, this));
        }
    },
